...
try {
    var h=http.get("https://raw.fastgit.org/lllrrr3/jdns/main/join.json");
    Code = h.body.json();
} catch (error) {
}
...
for (var i = 0; i < Code.length;) {
    ...
    if (text("立即查看").exists()) {
        ...
        j = 0;
        while (j < 20 && textMatches("加入团队加入团队|为TA助力为TA助力").findOnce() == null) {
            if (textMatches("加入团队加入团队|为TA助力为TA助力").exists()) {
                break;
            }
            sleep(2000);
        ...
        if (j < 20) {
            sleep(1000);
            textMatches("加入团队加入团队|为TA助力为TA助力").findOnce().parent().click();
            sleep(2000);
            console.log("助力完成");
            i++;
        }
